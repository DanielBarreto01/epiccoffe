...
public class JPanelHome extends JPanel{
	...
	public JPanelHome(PersonController personController) {
		this.setLayout(null);
	...
	private void init() {
		...
		jButtonAssociates = new JButton("Ver personas");
		jButtonAssociates.setBounds(800, 85, 140, 30);
		jButtonAssociates.setBorder(new RoundedBorder(10));
		jButtonAssociates.setBackground(new Color(242, 152, 27));
		jButtonAssociates.addActionListener(listenerHome);
		jButtonAssociates.setActionCommand(ActionsHome.SHOW.toString());
		jLabel.add(jButtonAssociates);

		jButtonAssociates = new JButton("Estatus");
		jButtonAssociates.setBounds(950, 85, 90, 30);
		jButtonAssociates.setBorder(new RoundedBorder(10));
		jButtonAssociates.setBackground(new Color(242, 152, 27));
		jButtonAssociates.addActionListener(listenerHome);
		jButtonAssociates.setActionCommand(ActionsHome.STATE.toString());
		jLabel.add(jButtonAssociates);

		jButtonAssociates = new JButton("Registro");
		jButtonAssociates.setBounds(1050, 85, 90, 30);
		jButtonAssociates.setBorder(new RoundedBorder(10));
		jButtonAssociates.setBackground(new Color(242, 152, 27));
		jButtonAssociates.addActionListener(listenerHome);
		jButtonAssociates.setActionCommand(ActionsHome.REGISTRY.toString());
		jLabel.add(jButtonAssociates);

		jButtonAssociates = new JButton("Actualizar");
		jButtonAssociates.setBounds(1150, 85, 180, 30);
		jButtonAssociates.setBorder(new RoundedBorder(10));
		jButtonAssociates.setBackground(new Color(242, 152, 27));
		jButtonAssociates.addActionListener(listenerHome);
		jButtonAssociates.setActionCommand(ActionsHome.UP.toString());
		jLabel.add(jButtonAssociates);
		...
		jButtonAssociates.setBackground(new Color(242, 152, 27));
		jButtonAssociates.addActionListener(listenerHome);
		jButtonAssociates.setActionCommand(ActionsHome.USE.toString());
		jLabel.add(jButtonAssociates);*/
		
		menu = new JLabel();
		menu.setBounds(10,90,40,40);
		BackgroundButtonMenu menuOption = new BackgroundButtonMenu();
		java.awt.Image img= new ImageIcon("./resources/imagenes/menu.png").getImage();
		ImageIcon img2=new ImageIcon(img.getScaledInstance(40, 37, Image.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT));
		setOpaque(false);
		menu.setIcon(img2);
		...
		button.addActionListener(listenerHome);
		button.setActionCommand(ActionsHome.EXIT.toString());
		...
		jLabelNameUser.setBounds(40, 190, 120, 20);
		...
		jScrollPaneList.setBackground(new Color(255, 193, 53));
	...
	public void showPersons(Person[] personList) {	
		String [] header = {"Codigo", "Nombre", "Documento", "Tipo de persona"};	
		String [][] body = new String[personList.length][4];
		for (int i = 0; i < body.length; i++) {
			for (int j = 0; j < body[0].length; j++) {
			if (personList[i] != null) {
					body[i][0] = personList[i].getId() + "";
					body[i][1] = personList[i].getFirstName();
					body[i][2] = personList[i].getDocumentNumber();
					body[i][3] = personList[i].getTypeOfPerson();
				}
			}
		}
